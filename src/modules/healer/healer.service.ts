...
export class HealerService {
  isAutoHealEnabled(argv: string[] = process.argv): boolean {
    if (argv.includes('--no-auto-heal')) return false;
    return process.env.AUTO_HEAL !== 'false';
  }

  getMaxHealAttempts(): number {
  ...
  createSession(
    ...
    return new HealSession(runId, configPath, variant, runDir, scenarioId);
  }

  backupConfig(configPath: string, variant: FlowVariant): string {
  ...
  applyFlowStepPatch(rules: TestRules, stepNumber: number, patch: ConfigPatch): TestRules {
    ...
    this.applyPatchToStep(step, patch);
    return cloned;
  }
  ...
  persistFlowStepPatch(
    ...
    if (scenarioId && parsed.scenarios) {
      ...
      const updated = this.applyFlowStepPatch(parsed as TestRules, stepNumber, patch);
